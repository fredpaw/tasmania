...
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videos SET  ordering = %d  WHERE ID = %d ", $_POST["order_by_".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videos SET  name = '%s'  WHERE ID = %d ", $_POST["titleimage".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videos SET  video_url_1 = '%s'  WHERE ID = %d ", $_POST["for_video_1".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videos SET  image_url = '%s'  WHERE ID = %d ", $_POST["imagess".$rowimages->id.""], $rowimages->id));
}
}
